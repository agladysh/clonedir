...
- Nobody but clonedir writes into a stage and nothing was published from it, so a dead run's stage is clonedir's own disposable output. It can still hold the only remaining copy of source content that changed or was deleted after the run; that is accepted. `--keep-failed` exists for anyone who wants a failed stage for diagnosis; it holds a partial, unverified copy, so it does not preserve the source. The `kept` marker is written when the stage is created, before any copy, because a SIGKILL or SIGHUP never reaches the failure path. A marked stage is never swept while it holds its tree; one without a tree (killed after publication or before copying) holds nothing and is swept normally. A successful clone removes its stage whether or not it was marked. Ownership is established by name prefix, uid and an owner record with a dead pid. A reused live pid keeps the stage (conservative). Pids are host-local, so parents shared across hosts or pid namespaces are out of scope.
